...
$g4['title'] = "Attendance Status";
include_once("./_head.php");

 if(!$is_admin){
 	alert("Only Admin Allowed here", $g4[path]);
 }
...
$sql = " 	select	a.mb_no, a.mb_id, a.mb_name, a.mb_1, a.mb_2, a.mb_3, b.mb_login_time,	
			...
			where 
					 a.mb_id <> 'admin'
			order by come desc, a.mb_1, a.mb_3 asc, a.mb_2 desc, a.mb_no desc
";
...
<div >
	<img src="<?=$g4[path]?>/images/attendance_status.png">
</div>
...
<tr valign=top>
    <td width=50% align=left><font size="3"><span class="blue">Local Status</span></font></td>
    <td width=50% align=right><font size="3"><span class="blue">Expat Status</span></font></td>
</tr>
<tr valign=top>
<td width=50%>
<table width="99%" cellpadding="0" cellspacing="1" class="normal_box" bgcolor="#dedede">
<colgroup width="33%">
<colgroup width="33%">
<colgroup width="33%">
<tr class='bgcol1 col1 ht center'>
...
$sql = "
		 select sum(notpresent + present) as total, sum(notpresent) as notpresent, sum(present) as present
		from (
			select	
						case 
							when ISNULL(b.mb_login_time)  then 1
							else 0
						end as  notpresent
						,case 
							when ISNULL(b.mb_login_time)  then 0
							else 1
						end as present
...
$result_b10 = sql_query($sql);

$colspan = 3;
for($i=0; $rowb=sql_fetch_array($result_b10); $i++) {
   	$list = $i%2;
	echo "<tr height='25' class='list$list col1 center'>
	<td><b>$rowb[total]</b></td>
	<td>$rowb[present]</td>
	<td>$rowb[notpresent]</td>";
	echo "</tr>";
}
