...
if ($sca || $stx) 
{
    ...
    if ($board['bo_search_level'] > $member['mb_level'])
        alert("you dont have the permission to search. please login first.", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table&sfl=$sfl&stx=$stx&sop=$sop"));
