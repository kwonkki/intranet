...
if (!$member[mb_id]) 
    alert_close("only member can use");

// 자동응답으로 설정되어 있는 경우에는 쪽지를 사용할 수 없습니다.
if (!$is_admin && $member['mb_memo_no_reply'] && $kind != "memo_config")
    alert("you need to change your configuration", "./memo.php?kind=memo_config");
