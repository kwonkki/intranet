...
{
    if ($cwin) // 코멘트 보기
       alert_close("board not existing -1", $g4[path]);
    else
       alert("board not existing -2", $g4[path]);
}
...
if ($wr_id) 
{
    ...
    {
        if (!$member[mb_id]) {
            $msg = "only for member view. please register first.";
            if ($cwin)
                alert_close($msg);
            else 
        ...
        else 
        {
            ...
            $row = sql_fetch($sql);
            if (!$row[cnt]) 
                alert("gruop access - no gruop acess", $g4[path]);
        }
    }
    ...
    if ($member[mb_level] < $board[bo_read_level]) 
    {
        if ($member[mb_id]) 
            ...
            alert("you level no permission to read", $g4[path]);
        else 
            alert("no permission to read. please login first", "./login.php?wr_id=$wr_id{$qstr}&url=".urlencode("./board.php?bo_table=$bo_table&wr_id=$wr_id"));
    }
...
else 
{
    ...
    {
        if ($member[mb_id]) 
            alert("no permission to read - 4", $g4[path]);
        else 
            alert("no permission to read - 5\\n please login first.", "./login.php?wr_id=$wr_id{$qstr}&url=".urlencode("board.php?bo_table=$bo_table&wr_id=$wr_id"));
    }

    if (!$page) $page = 1; 

    $g4[title] = "$board[bo_subject] $page page";
}
...
switch($mnb){
	default:
		$title_img = "<img src=\"{$g4[path]}/images/default.png\"  />";
	break;
}   
...
echo "<script type=\"text/javascript\" src=\"$g4[path]/js/jquery.krioImageLoader.js\"></script>\n";

if (!($board[bo_use_comment] && $cwin)) {
    // 게시물 아이디가 있다면 게시물 보기를 INCLUDE
    if ($wr_id || $bo_table == "daily_report") 
