...
if (empty($_POST))
    alert("file or content size is bigger than the configuration.\\n\\npost_max_size=".ini_get('post_max_size')." , upload_max_filesize=$upload_max_filesize\\n\\nplease contact the administrator.");
...
if ($w == "u" || $w == "r") {
    ...
    if (!$wr[wr_id])
        alert("no article existing", $goto_url); 
}
...
if ($w == "" || $w == "u") {
    ...
    if($w =="u" && $member['mb_id'] && $wr['mb_id'] == $member['mb_id'])
        ...
        alert("no permission to write.", $goto_url);

	// 외부에서 글을 등록할 수 있는 버그가 존재하므로 공지는 관리자만 등록이 가능해야 함
	if (!$is_admin && $notice)
		alert("only admnistrator to write the notice.", $goto_url);
} 
else if ($w == "r") 
{
    if (in_array((int)$wr_id, $notice_array))
        alert("you cant reply in the notice.", $goto_url);

    if ($member[mb_level] < $board[bo_reply_level]) 
        alert("you dont have permission to reply.", $goto_url); 
    ...
    if (strlen($reply_array[wr_reply]) == 10)
        alert("you cant reply anymore. only 10 reply available.", $goto_url);
    ...
    if (!$row[reply])
        ...
        alert("you cant reply anymore. only 26 reply available.", $goto_url);
    else
        $reply_char = chr(ord($row[reply]) + $reply_number);

    $reply = $reply_array[wr_reply] . $reply_char;
} else 
    alert("no wr_id value.", $goto_url); 


if ($w == "" || $w == "r") 
{
    if ($_SESSION["ss_datetime"] >= ($g4[server_time] - $config[cf_delay_sec]) && !$is_admin) 
        alert("you cant write too fast.", $goto_url);
    ...
    if ($row[prev_md5] == $curr_md5 && !$is_admin)
        alert("content are same.", $goto_url);
} 
...
if (!$is_member) {
    if ($w=='' || $w=='r') {
        ...
        if ( !zsfCheck( $_POST['wr_key'], $_GET['bo_table'] ) ) { alert ('spam code wrong.', $goto_url); }    
    }
}

if (!isset($_POST[wr_subject]) || !trim($_POST[wr_subject])) 
    alert("please type the title.", $goto_url); 
...
for ($i=0; $i<count($_FILES[bf_file][name]); $i++) 
{
    ...
    if ($filename)
    {
        ...
        {
            $file_upload_msg .= "\'{$filename}\' size is bigger than the maximum size ($upload_max_filesize)\\n";
            continue;
        }
        else if ($_FILES[bf_file][error][$i] != 0)
        {
            $file_upload_msg .= "\'{$filename}\' is no uploaded correctly.\\n";
            continue;
        }
    }
    ...
    if (is_uploaded_file($tmp_file)) 
    {
        ...
        {
            $file_upload_msg .= "\'{$filename}\' file size (".number_format($filesize)." is bigger than the maximum uploading size (".number_format($board[bo_upload_size]).".\\n";
            continue;
        }
...
if ($w == "" || $w == "r") 
{
    ...
    else 
    {
        ...
        if (!trim($wr_name))
            alert("please type the name.", $goto_url);
...
else if ($w == "u") 
{
    ...
    if ($is_admin == "super") // 최고관리자 통과
        ...
        if ($member[mb_id] != $group[gr_admin]) // 자신이 관리하는 그룹인가?
            alert("you are not allowed to changed. - 1", $goto_url);
        else if ($member[mb_level] < $mb[mb_level]) // 자신의 레벨이 크거나 같다면 통과
            alert("you cant change the higher level article. - 3", $goto_url);
    } else if ($is_admin == "board") { // 게시판관리자이면
        $mb = get_member($write[mb_id]);
        if ($member[mb_id] != $board[bo_admin]) // 자신이 관리하는 게시판인가?
            alert("you are not allowed to changed - 4.", $goto_url);
        else if ($member[mb_level] < $mb[mb_level]) // 자신의 레벨이 크거나 같다면 통과
            alert("you are not allowed to changed - 5.", $goto_url);
    } else if ($member[mb_id]) {
        if ($member[mb_id] != $write[mb_id])
            alert("this is not your article.", $goto_url);
    } else {
        if ($write[mb_id]) {
            alert("After login, you can update this.", "./login.php?url=".urlencode("./board.php?bo_table=$bo_table&wr_id=$wr_id"));
        }
    }
