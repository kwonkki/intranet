...
{
    if ($cwin) // 코멘트 보기
       alert_close("no board -1.", $g4[path]);
    else
       alert("no board -2.", $g4[path]);
}

if (!$bo_table)
    alert("no bo_table parameter - 3.", $g4[path]);
...
if ($board['bo_singo_nowrite']) {
    ...
    if ($singo_result)
        alert("[alert] you dont have permission to write", "$g4[bbs_path]/board.php?bo_table=$bo_table");
}
...
if ($w == "" && $board[bo_realcheck] && !$is_admin) { 
    if ($member[mb_realcheck] == "0000-00-00 00:00:00") { 
        alert("no permission -5 ", "$g4[path]/cb/index.php"); 
    } 
}
...
if ($w == "") 
{
    ...
    if ($member[mb_level] < $board[bo_write_level]) {
        if ($member[mb_id])
            ...
            alert("no permission.\\n\\n if you are member, please log in first.", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
    }
...
else if ($w == "u")
{
    ...
    $row = sql_fetch($sql);
    if ($row[cnt] && !$is_admin)
        alert("this one has reply. so, content not allowed to changed.");
*/
    // 코멘트 달린 원글의 수정 여부
    $sql = " select count(*) as cnt from $write_table
    ...
    if ($row[cnt] >= $board[bo_count_modify] && !$is_admin)
        alert("if this one has comment more than  {$board[bo_count_modify]}, you are not allowed to change");
...
else if ($w == "r")
{
    if ($member[mb_level] < $board[bo_reply_level]) {
        if ($member[mb_id])
            ...
            alert("no permission to reply - 2", "./login.php?$qstr&url=".urlencode("$_SERVER[PHP_SELF]?bo_table=$bo_table"));
    }
...
<script type="text/javascript"> 
  ...
  $(window).bind('beforeunload', function(){ 
      if ( btn_submit_pressed ) 
          return 'if you exit, your write will not be saved.'; 
  });
  */
