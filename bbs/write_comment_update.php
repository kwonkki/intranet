...
$g4[title] = $wr_subject . "comment uypdate";
...
if ($w == "c" && $_SESSION["ss_datetime"] >= ($g4[server_time] - $config[cf_delay_sec]) && !$is_admin) 
    alert("you cant upload many too fast.");
...
if (!$is_member) {
    if ($w=='' || $w=='c') {
        ...
        if ( !zsfCheck( $_POST['wr_key'], $_GET['bo_table'] ) ) { alert ('spam code wrong.'); }    
    }
}
