...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($start_page > 1) $str .= " &nbsp;<a href='" . $url . ($start_page-1) . "{$add}'>previous</a>";
...
function get_list($write_row, $board, $skin_path, $subject_len=40, $gallery_view=0)
{
	...
	foreach ($list as $key => $value) {
	    ChromePhp::log("list value : ". $key, $value);
	}	
	*/
...
function insert_vacation_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='')
{
    global $config;
    global $g4;
    global $is_admin;
    ...
    if ($rel_table || $rel_id || $rel_action)
    {
        ...
        $sql = " select count(*) as cnt from $g4[vacation_point_table]
                  where mb_id = '$mb_id'
                    and po_rel_table = '$rel_table'
                    and po_rel_id = '$rel_id'
                    and po_rel_action = '$rel_action' ";
    ...
    */
    // 포인트 건별 생성
    $sql = " insert into $g4[vacation_point_table]
                set mb_id = '$mb_id',
                    po_datetime = '$g4[time_ymdhis]',
                    po_content = '".addslashes($content)."',
                    po_point = '$point',
...
//ChromePhp::log($sql);
    // 포인트 UPDATE
    $sql = " update $g4[member_table] set mb_vacation_point = mb_vacation_point + '$point' where mb_id = '$mb_id' ";
    sql_query($sql);
//ChromePhp::log($sql);
    return 1;
}
