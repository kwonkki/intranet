...
// Choice Comment
$sql = " select * from $write_table where wr_parent = '$wr_id' and wr_is_comment = 1 and wr_5 like '%choice%' order by wr_good ";
$result = sql_query($sql);

?>

<table border=0 cellpadding=0 cellspacing=0 width=100%>
<tr><td class="cmt_choice_head"><?=$amina[cmt_choice_txt]?>된 댓글</td></tr>
<tr><td class="cmt_choice_body">
		...
		<td valign=top>
			...
			<td align=right>
				<? if($amina[cmt_good]) { ?>
					...
					<? if($amina[cmt_best] > 0 && $row[wr_good] >= $amina[cmt_best]){ ?><div style='float:right;'><span class="cmt_best_icon">베플</span>&nbsp;</div><? } ?>
				<? } ?>
			</td></tr>
