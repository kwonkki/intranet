...
<table width="100%" cellpadding="0" cellspacing="0" class="gallery_tbl">
<tr class="head_tr">
<td colspan="<?=$set[list_mod]?>" style='font:1px;padding:0;margin:0;line-height:0'>&nbsp;</td>
</tr>
...
?>
		<tr>
		<td colspan="<?=$set[list_mod]?>" class='notice'>
	...
	?>
		<? if ($n == 0) { ?><tr><? } ?>
	    <? if ($n>0 && (($n%$set[list_mod])==0) ) { ?></tr><? } ?>
		<td align=center valign=top width='<?=$td_width?>%'>
			<table cellpadding=0 cellspacing=0 width="<?=$set[list_thumb_w]?>" class="list_tbl">
			<? if ($is_thumb) { 
				$thumb_img = "";
				$thumb_icon = "";
				if(!$is_blind) {
			...
			<? if ($is_checkbox) { ?><tr><td align=center><input type=checkbox name=chk_wr_id[] value="<?=$list[$i][wr_id]?>"></td></tr><? } ?>
			</table>
		</td>
	<? $n++; } ?>
...
<? // 나머지 td 를 채운다.
	if (($cnt = $n%$set[list_mod]) != 0) {
		for ($j=$cnt; $j<$set[list_mod]; $j++) { 
?>
		<td>&nbsp;</td>
<? } } ?> 

<? if ($k == 0) { ?><td colspan='<?=$set[list_mod]?>' class='no_list'><?=$no_list?></td><? } ?>
</tr>
<tr class='end_line'><td colspan='<?=$set[list_mod]?>' class='end_line'>&nbsp;</td></tr>
</table>
