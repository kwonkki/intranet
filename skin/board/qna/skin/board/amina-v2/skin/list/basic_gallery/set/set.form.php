...
<tr>
<td align=center class="td3">가로수</td>
<td>
	<? if(!$set[list_mod]) $set[list_mod] = 3; ?>	
	<input type=text class=ed name='list_mod' size=3 maxlength=10 numeric value='<?=$set[list_mod]?>'> 열로 표시 
</td>
</tr>
<tr>
...
<td>
	크기 
	<? if(!$set[list_thumb_w] || $set[list_thumb_w] == "" ) $set[list_thumb_w] = 260; ?>
	<input type=text class=ed name='list_thumb_w' size=3 maxlength=10 numeric value='<?=$set[list_thumb_w]?>'>

	<? if($set[list_thumb_h] == "") $set[list_thumb_h] = 140; ?>	
...
<td>
	<? if($set[subj_len] == "") $set[subj_len] = 50; ?>
...
<td>
	<? if($set[conts_len] == "") $set[conts_len] = 110; ?>
...
<td>
	...
	<select name="info_align">
		...
		<option value="left" <? if($set[info_align] == "left") { ?> selected<? } ?>>좌측</option>
	</select>
