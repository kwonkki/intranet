...
var gup_path = g4_path + '/guploader';
...
this.set_path = function(get_path) {
    gup_path = get_path;
}
...
this.draw_uploader = function() {
    ...
    draw += "<td width=0 id="+gup_name+"_image_preview></td><td width=0></td>";
    ...
    draw += "<param name=\"movie\" value=\""+gup_path+"/flash_uploader.swf\" />";
    ...
    draw += "<embed src=\""+gup_path+"/flash_uploader.swf\" quality=\"high\" bgcolor=\"#ffffff\" wmode=\"transparent\"";
    ...
    draw += "</embed></object>";
    draw += "</td></tr></table></td></tr><tr><td></td><td></td><td height=0 align=right>";
    draw += "&nbsp;&nbsp;";
    draw += "</td></tr></table>";
    document.write(draw);
        ...
        if (fl.options[i].selected==true) {
            ...
            var myAjax = new Ajax.Request(
                url, 
                {
                    method: 'post', 
                    parameters: param,
                    onComplete: eval(gup_name + ".delete_file_complete")
                });
        }
    }
}
...
this.get_file_info = function(val) {
    var arr = val.split('|');
    var ret = {"bf_no":arr[0], "save_name":arr[1], "real_name":arr[2], "size":arr[3]};
    return ret;
}
