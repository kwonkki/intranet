...
quvic.listVideosCall = function(json) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var tubeUrl  = '/skin/write_tube.php?vid='+videoID+'&title='+addslashes(title)+'&duration='+duration+'&thumbUrl='+thumbnailUrl+'&viewCount='+viewCount+'&keywords='+keywords;
	...
	html.push('</span><br/>');
	html.push('<a href="javascript:win_youtube(\''+tubeUrl+'\');" class="poket">[등록]</a>', title.substr(0,25), '');
	html.push('</dt>');
	}
...
// 스크랩 창
function win_youtube(url)
{
	if (!url) return;

	win_open(url, "youtube", "left=20, top=20, width=480, height=500, scrollbars=1");
}