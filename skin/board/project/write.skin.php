...
<? if ($is_category) { ?>
<tr>
    <td class=write_head>팀 분류</td>
    <td><select name=ca_name required itemname="분류"><option value="">선택하세요<?=$category_option?></select></td></tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>
<? } ?>

<tr>
    <td class=write_head>프로젝트명</td>
    <td colspan=5><input class='ed' style="width:100%;" name=wr_subject id="wr_subject" itemname="제목" required value="<?=$subject?>"></td></tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>

<tr>
    <td class=write_head>시작일</td>
    <td>
		<input class=ed type=text id=wr_1 name='wr_1' size=8 maxlength=8 minlength=8 required numeric itemname='시작일' value='<?=$write[wr_1]?>' readonly title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.' onClick="javascript:win_calendar('wr_1', document.getElementById('wr_1').value, '');">
...
<tr>
    <td class=write_head>마감일</td>
    <td>
		<input class=ed type=text id=wr_2 name='wr_2' size=8 maxlength=8 minlength=8 required numeric itemname='마감일' value='<?=$write[wr_2]?>' readonly title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.' onClick="javascript:win_calendar('wr_2', document.getElementById('wr_2').value, '');">
        <a href="javascript:win_calendar('wr_2', document.getElementById('wr_2').value, '');"><img src='<?=$board_skin_path?>/img/calendar.gif' border=0 align=absmiddle title='달력 - 날짜를 선택하세요'></a>
	</td>
</tr>

<tr>
    <td class=write_head>담당자</td>
    <td><input class='ed' style="width:100px;" name=wr_3 id="wr_3" itemname="담당자" required value="<?=$write[wr_3]?>"></td>
</tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>
<tr>
	<td class=write_head>비고</td>
    <td class='write_head' style='padding:5 0 5 10;' colspan='1'>
        <? if ($is_dhtml_editor) { ?>
