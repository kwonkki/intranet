...
<? for ($i=0; $i<count($list); $i++) { 
	...
	if(  ($list[$i][wr_1] == $member[mb_id]) || $is_admin ){
?>
<tr height=28 align=center> 
    <td>
        <? 
    ...
    <td><? if($list[$i][wr_5]==""){  ?> <input type=checkbox name=chk_wr_id[] value="<?=$list[$i][wr_id]?>"> <? } ?></td>
    ...
    <td align=left style='word-break:break-all;'>
		...
		$link2 = $list[$i][href]."&review_auth=1";

        echo "<a href='{$link2}' $style>";
        echo $list[$i][subject];
        echo "</a>";
        ...
        echo $nobr_end;
        ?></td>
    <td><nobr style='display:block; overflow:hidden; width:100px;'><b><?=$list[$i][wr_1]?></b></nobr></td>
...
<tr> 
    ...
    <td width="50%" align="right">
            <span class="button"><input type="button" value="Finish The Review" class="btn1" onclick="javascript:select_delete(); " ></span>
    </td>
</tr>
</table>
</form>
...
function select_delete() {
    ...
    if (!confirm("Have you finished reviewing the report?"))
        return;

    f.action = "./review_all.php";
    f.submit();
}
