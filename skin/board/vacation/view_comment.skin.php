    ...
    <td width='100%'>
        ...
        <tr>
            <td valign=top>
                ...
                <?
                //if (strstr($list[$i][wr_option], "secret") and ($list[$i][mb_id] == $member[mb_id] or $is_admin or $member[mb_id] == $write[mb_id])) echo "<span style='color:#ff6600;FONT-WEIGHT:bold'>*secret</span><BR> ";
                if (strstr($list[$i][wr_option], "secret")) echo "<span style='color:#ff6600;FONT-WEIGHT:bold'>*secret</span><BR> ";
...
function fviewcomment_submit(f)
{
    ...
    {
        alert("forbidden word included ('"+s+"')");
        <? if (!$is_dhtml_editor) { ?>
        f.wr_content.focus();
        <? } ?>
        return false;
    ...
    if (f.char_count) {
        if (char_min > 0 || char_max > 0) {
            ...
            if (char_min > 0 && char_min > cnt) {
                alert("comment should be more than "+char_min+" character");
                return false;
            } 
            else if (char_max > 0 && char_max < cnt) {
                alert("comment maximum character would be "+char_max+" character");
                return false;
            }
        }
    }
...
function post_submit(action_url, bo_table, wr_id, comment_id, flag, msg)
{
  ...
  var submit_msg = msg + " - want to executed?";
