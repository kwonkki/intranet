                ...
                if ($cnt > $dt[wday])
                {
                    ...
                    if ($day <= $last_day)
                    {
                        ...
                        if ($today[year] == $yyyy && $today[mon] == $mm && $today[mday] == $day)
                        {
                            ...
                            echo "<script language='JavaScript'>document.getElementById('id$i$k').title+='[today]';</script>";
                        }
                        // 선택일(넘어온 값) 이라면
                        else if ($tmp_date == $cur_date)
                        {
                            ...
                            echo "<script language='JavaScript'>document.getElementById('id$i$k').title+='[selected day]';</script>";
                        }
                    } else
                        echo "&nbsp;";
                } else
...
<tr>
    <td align=center height=30>
        ...
        today : <?="{$today[year]}yr {$today[mon]}mon {$today[mday]}day ({$yoil[$today[wday]]})";?></a>
        </span></td>
</tr>
</form>
</table>
