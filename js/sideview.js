if (typeof(SIDEVIEW_JS) == 'undefined') // 한번만 실행
{
    if (typeof g4_is_member == 'undefined')
        alert('g4_is_member is not defiend. js/sideview.js');
    if (typeof g4_path == 'undefined')
        alert('g4_path is not defiend. js/sideview.js');
    ...
    function renderRow() 
    {
        ...
        var str = "<tr height='30'><td id='sideViewRow_"+this.name+"'>&nbsp;<font color=gray>&middot;</font>&nbsp;<span style='color: #A0A0A0;  font-family: 돋움; font-size: 11px;'>"+this.onclickEvent+"</span></td></tr>";
        return str;
    }
    ...
    function SideView(targetObj, curObj, mb_id, name, email, homepage, domain) 
    {
        ...
        /*
            if (mb_id) 
                ...
                this.insertTail("memo", "<a href=\"javascript:win_memo('"+g4_path+"/" + g4_bbs + "/memo.php?kind=write&me_recv_mb_id="+mb_id+"', '" + mb_id + "', '" + domain + "');\">message</a>"); 
            // 메일보내기
            if (email) 
                this.insertTail("mail", "<a href=\"javascript:;\" onclick=\"win_formmail('"+mb_id+"','"+name+"','');\">sending email</a>");
            ...
            // 자기소개
            if (mb_id) 
                this.insertTail("info", "<a href=\"javascript:;\" onclick=\"win_profile('"+mb_id+"');\">introduction</a>");
        //}

        // 게시판테이블 아이디가 넘어왔을 경우
        if (g4_bo_table) {
            if (mb_id) // 회원일 경우 아이디로 검색
                this.insertTail("mb_id", "<a href='"+g4_path+"/" + g4_bbs + "/board.php?bo_table="+g4_bo_table+"&sca="+g4_sca+"&sfl=mb_id,1&stx="+mb_id+"'>search by id</a>");
        ...
        if (mb_id) {
            this.insertTail("new", "<a href='"+g4_path+"/" + g4_bbs + "/new.php?mb_id="+mb_id+"'>all article</a>");
        }
        /*
        if (g4_sms4) {
            this.insertTail("hp", "<a href='"+g4_path+"/sms/?mb_id="+mb_id+"'>sending sms</a>");
        }
        */
        ...
        if (g4_is_admin == "super") {
            // 회원정보변경
            if (mb_id)
                this.insertTail("modify", "<a href='"+g4_path+"/" + g4_admin + "/member_form.php?w=u&mb_id="+mb_id+"' target='_blank'>change user-info</a>");
            // 포인트내역
            if (mb_id)
                this.insertTail("point", "<a href='"+g4_path+"/" + g4_admin + "/point_list.php?sfl=mb_id&stx="+mb_id+"' target='_blank'>point history</a>");
        }
        */
    }
    ...
    {
        var str = "<table border='0' cellpadding='0' cellspacing='0' width='130' style='border:1px solid #E0E0E0;' bgcolor='#F9FBFB'>";
