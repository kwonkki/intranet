...
if (!$g4_path || preg_match("/:\/\//", $g4_path))
    die("<meta http-equiv='content-type' content='text/html; charset=$g4[charset]'><script type='text/javascript'> alert('your parameter something wrong.'); </script>");
//if (!$g4_path) $g4_path = ".";
$g4['path'] = $g4_path;
...
*/
    ...
    if (!$select_db)
        die("<meta http-equiv='content-type' content='text/html; charset=$g4[charset]'><script type='text/javascript'> alert('DB access error'); </script>");
...
if ($_SESSION['ss_mb_id']) // 로그인중이라면
{
    ...
    {
        // 첫 로그인 포인트 지급
        insert_point($member['mb_id'], $config['cf_login_point'], "{$g4['time_ymd']} first login", "@login", $member['mb_id'], $g4['time_ymd']);
...
if ($is_admin != "super") {
    ...
    if ($cf_possible_ip) {
        ...
        if (!$is_possible_ip)
            die ("you are not allowed to access. -1");
    }
    ...
    for ($i=0; $i<count($pattern); $i++) {
        ...
        if ($is_intercept_ip)
            die ("you are not allowed to access -2.");
    }
}
