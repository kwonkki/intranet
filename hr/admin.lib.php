    ...
    if ($mb[mb_level] >= 1) {
        $sql = " update $g4[member_table] 
                    set mb_jumin = '',
                        ...
                        mb_sex = '',
                        mb_signature = '',
                        mb_memo = '".date("Ymd",$g4['server_time'])." deleted\n\n$mb[mb_memo]',
...
function auth_check($auth, $attr)
{
    ...
    if (!strstr($auth, $attr)) {
        if ($attr == "r")
            alert("no auth to read.");
        else if ($attr == "w")
            alert("no auth to anything.");
        else if ($attr == "d")
            alert("no auth to delete.");
        else 
            alert("configuration wrong.");
    }
}
...
function textarea_size($fld) 
{
    ...
    $s .= "<span onclick=\"javascript:textarea_size(document.getElementById('$fld'), {$size})\"><img src='$g4[hr_path]/img/btn_up.gif' border=0 align=absmiddle></span> ";
    $s .= "<span onclick=\"javascript:textarea_size(document.getElementById('$fld'), ".$size*(-1).")\"><img src='$g4[hr_path]/img/btn_down.gif' border=0 align=absmiddle></span>";
    $s .= "&nbsp;&nbsp;</td></tr></table>";
    return $s;
}
...
function icon($act, $link="", $target="_parent")
{
    ...
    $icon = "<img src='{$g4[hr_path]}/img/icon_{$img[$act]}.gif' border=0 align=absmiddle title='$act' width=22 height=21>";
    if ($link)
...
function help($help="", $left=0, $top=0)
{
    ...
    $str  = "<img src='$g4[hr_path]/img/icon_help.gif' border=0 width=15 height=15 align=absmiddle onclick=\"help('help$idx', $left, $top);\" style='cursor:hand;'>";
...
function subtitle($title, $more="") 
{
    ...
    $s = "<table width=100% cellpadding=0 cellspacing=0><tr><td width=80% align=left><table border='0' cellpadding='0' cellspacing='1'><tr><td height='24'><img src='$g4[hr_path]/img/icon_title.gif' width=20 height=9> <font color='#525252'><b>$title</b></font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table><table width=100% cellpadding=0 cellspacing=0><tr><td height=1></td></tr></table></td><td width=20% align=right>";
    if ($more)
        $s .= "<a href='$more'><img src='$g4[hr_path]/img/icon_more.gif' width='43' height='11' border=0 align=absmiddle></a>";
    $s .= "</td></tr></table>\n";
...
if (!$member['mb_id'])
{
    ...
    alert("login first please.", "$g4[bbs_path]/login.php?url=" . urlencode("$_SERVER[PHP_SELF]?$_SERVER[QUERY_STRING]"));
}
else if ($is_admin != "super") 
{
    ...
    if (!$i)
    {
        alert("administrator can access here.", $g4[path]);
    }
}
